...
posts.postId, 
posts.message,
(SELECT Count(DISTINCT clapId) FROM claps WHERE feedId = feeds.feedId) AS count_clap, 
(SELECT Count(DISTINCT commentId) FROM comments WHERE feedId = feeds.feedId) AS count_comments, 
(SELECT Count(DISTINCT shareId) FROM shares WHERE feedId = feeds.feedId) AS count_share
FROM users
