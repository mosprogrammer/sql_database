...
--
-- Host: localhost:3306
-- Generation Time: 31 พ.ค. 2020 เมื่อ 07:13 PM
...
--
-- Stand-in structure for view `thailand_provices`
-- (See below for the actual view)
--
CREATE TABLE `thailand_provices` (
`ProviceName` varchar(150)
,`AmphureName` varchar(150)
...
--
-- Structure for view `thailand_provices`
--
DROP TABLE IF EXISTS `thailand_provices`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `thailand_provices`  AS  select `p`.`name_th` AS `ProviceName`,`a`.`name_th` AS `AmphureName`,`d`.`name_th` AS `DistrictName`,`d`.`zip_code` AS `ZipCode` from ((`provinces` `p` join `amphures` `a` on((`a`.`province_id` = `p`.`id`))) join `districts` `d` on((`d`.`amphure_id` = `a`.`id`))) ;
